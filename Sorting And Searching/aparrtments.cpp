...
ll int n,m,k;
cin>>n>>m>>k;
ll int applicant[n],apartment[m];
for(int i =0;i<n;i++) cin>>applicant[i];
for(int i =0;i<m;i++) cin>>apartment[i];
sort(applicant,applicant+n);
priority_queue<ll int>pq;
for(int i = 0;i<m;i++) pq.push(apartment[i]);
ll int ans = 0;
for(int i = n-1;i>=0;i--){
    if(pq.empty()) break;
    else{
        while(!pq.empty() && pq.top()>applicant[i]+k) pq.pop();
        if(pq.empty()) break;
        if(pq.top()<=applicant[i]+k && pq.top()>=applicant[i]-k){
            ans++;
            pq.pop();
        }
    }
